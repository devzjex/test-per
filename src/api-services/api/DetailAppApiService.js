...
export default class DetailAppApiService {
    ...
    static async getSubscriptionByPlan(id, fromDate, toDate) {
        const api = getDataByDate('partner/subscription_by_pricing/', id, fromDate, toDate);
        const result = await CommonCall(api);
        return result;
    }
    ...
    static async getDetailNumberReinstallShopByTime(id, period) {
        const result = await CommonCall(
            `${URL_API}partner/detail_number_reinstalled_shop_by_time/${id}?period=${period}`,
        );
        return result;
    }
