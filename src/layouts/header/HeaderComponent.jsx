...
const HeaderComponent = ({
    ...
    const showOnboard = initialTourGuide.show_onboarding;
    ...
    useEffect(() => {
        const processedNotifications = processNotifications(initialNotifications.notifications);
    ...
    const resourceSubmenu = (
        <Menu className={styles.appsDropdown}>
            {MENU_RESOURCE_ITEM.map((item) => (
                <Menu.Item
                    key={item.key}
                    className={`${currentKey === item.linkTo ? styles.activeItem : styles.noActive}`}
                >
                            ...
                            {item.submenu.map((subItem) => {
                                return (
                                    ...
                                    >
                                        <MyLink
                                            href={subItem.linkTo}
                                            onClick={() => handleClickMenuItem(subItem.linkTo)}
                                        >
                                            {subItem.title}
                                        </MyLink>
                                    </Menu.Item>
                                ...
                                <Menu.Item style={{ backgroundColor: renderBgColor(item.detail) }}>
                                    ...
                                    {item.detail.built_for_shopify === true && (
                                        <>
                                            <Tooltip
                                                title={
                                                    item.detail && item.detail.rank_bfs > 0 ? item.detail.rank_bfs : ''
                                                }
                                            >
                                                <Image
                                    ...
                                    {Object.keys(item.changed).length > 0 &&
                                        item.changed.review_count &&
                                        !item.watched_changes && (
                                            <span className={styles.reviewChange}>
                                                {getReviewChanges(
                                                    item.changed.review_count.after,
                                                    item.changed.review_count.before,
                                                )}
                                            </span>
                                        )}
                                    ...
                                    >
                                        ...
                                        <Menu.Item key="insights" className={styles.menuItemDetail}>
                                            ...
                                            >
                                                <span
                                                    className={`${styles.menuLink} ${
                                                        isActive(currentKey, ['/developers', '/dashboard/reviews'])
                                                            ? styles.active
                                                            : ''
                                                    }`}
                                                >
                                        ...
                                        <Menu.Item key="resources" className={styles.menuItemDetail}>
                                            <Dropdown placement="bottom" overlay={resourceSubmenu} arrow>
                                                <span
                                                    className={`${styles.menuLink} ${currentKey === '/blogs' ? styles.active : ''}`}
                                                >
                                                    Resources
                                                    <CaretDownOutlined style={{ marginLeft: '5px' }} />
                                                </span>
                        ...
                        {isShowProfile ? (
                            <div className={`${styles.headerProfile} flex items-center`}>
                                ...
                                >
                                    ...
                                    >
                                        <Avatar
                                            className={styles.avatarProfileHeader}
                                            style={{ backgroundColor: '#FFC225' }}
                                        >
                                            {fullName ? fullName.substring(0, 1).toLocaleUpperCase() : ''}
                                        </Avatar>
                                        <MoreOutlined />
                                    </Tooltip>
                                </Dropdown>
                            ...
                            <div className={styles.registerLogin}>
                                <div className={styles.buttonRegister}>
                                    <MyLink
                                        className={styles.buttonRegisterStyled}
                                        href={`${LayoutPaths.Auth}${Paths.Register}`}
                                    >
                                        Get Started
                                    </MyLink>
                                </div>
                                <div className={styles.buttonLogin}>
                                    <MyLink
                                        className={styles.buttonLoginStyled}
                                        href={`${LayoutPaths.Auth}${Paths.LoginApp}`}
                                    >
                                        Login
                                    </MyLink>
                                </div>
