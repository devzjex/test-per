...
export default function ModalAddKeyword(props) {
    ...
    const fetchSuggestKeyword = async (id) => {
        ...
        if (result && result.code == 0) {
            setData(
                result.keywords.filter((item) => !props.keywordExist.map((key) => key.keyword.keyword).includes(item)),
            );
        }
    };
    ...
    const onFinish = async (values) => {
        ...
        if (hasExist) {
            message.error(`Keyword "${existKeyword}" already exists!`);
            form.setFieldsValue({
                keyName: keywords.filter((k) => k !== existKeyword),
            });
            return;
        }
                    ...
                    <Form form={form} layout="vertical" className="form-edit" onFinish={onFinish}>
                        ...
                        {data && data.length > 0 && (
                            <Form.Item className="suggest-keyword">
                                <div className="suggest-keyword-note">
                                    * List of keywords related obtained from AI bot
                                </div>
                                <>
                                    {data.map((item, index) => (
                                        <Tag
