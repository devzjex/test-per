...
function Retention(props) {
    ...
    const stageType = useMemo(() => {
        ...
        const array = Array.from(
            { length: dateLength > 40 ? Math.floor(dateLength / 31 + 1) : dateLength },
            (_, index) => {
    ...
    useEffect(() => {
        ...
        setColumDate(column);
        setDataRetention(
            dataDays.map((item) => item.slice(stage.start, stage.end + 1)).filter((ele) => ele.length !== 0),
        );
    ...
    const showNumberMerchants = (totalMerchant, merchant, type) => {
        ...
        if (type == 'retention') {
            ...
            totalMerchant - merchant == 1 || totalMerchant - merchant == 0
                ? (text = ' merchant')
                : (text = ' merchants');
                                ...
                                {dataRetention.map((item, index) => {
                                    return (
                                        <div key={index + item} className="content-retention-table-content">
                                            <div className="title-retention">
                                                {columDate[index].title} <br />{' '}
                                                <span>{columDate[index].users} merchants</span>
                                            </div>
                                            {item.map((value, index2) => {
                                                return (
                                                    <div
                                                        key={index2 + value}
                                                        onClick={() =>
                                                            showListShop(value, stageActive * 30 + index2 + 1)
                                                        }
                                                        title={showNumberMerchants(
                                                            columDate[index].users,
                                                            value.usersRetention,
                                                            type,
                                                        )}
