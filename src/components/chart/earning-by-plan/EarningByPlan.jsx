...
export default function EarningByPlan(props) {
  ...
  const data = useMemo(() => {
    ...
    const sumCount = valueEarning
      ? valueEarning.filter((item) => item.type !== 'other').reduce((acc, item) => acc + item.price * item.count, 0)
      : 0;
  ...
  const dataSubscription = useMemo(() => {
    ...
    return uniquePrices.map((priceMonthly) => {
      ...
      return {
        ...
        percent_monthly_subscription: monthlySubscription ? `${percent_monthly_subscription.toFixed(2)}%` : '0%',
              ...
              <thead>
                ...
                <tr>
                  {headersMonthly.map(({ key, label, tooltip }) => (
                    <th key={key} onClick={() => handleSortMonthly(key)} style={{ cursor: 'pointer' }}>
                      {label} {getTooltipContent(tooltip)}{' '}
                      <SortIcon active={sortColumnMonthly === key} direction={sortDirectionMonthly} />
                    </th>
                  ))}
                </tr>
              </thead>
                        ...
                        <tr key={index}>
                          ...
                          <td>
                            <div className="double-value-cell">
                              {item.merchants_monthly_subscription === 0 && item.merchants_monthly === 0 ? (
                                <span></span>
                              ) : (
                                <>
                          ...
                          <td>
                            <div className="double-value-cell">
                              {item.rev_monthly_subscription === 0 && item.rev_monthly === 0 ? (
                                <span></span>
                              ) : (
                                <>
                                  <span>${item.rev_monthly.toLocaleString('en-US')}</span>
                                  <div className="divider" />
                                  <span>${item.rev_monthly_subscription.toLocaleString('en-US')}</span>
                                </>
                              )}
                            </div>
                          </td>
                    ...
                    <>
                      ...
                      {hasAnyData && (
                        <tr className="sum-row">
                          <td>
                            {!(
                              totalPriceMonthlyEarningNegative === 0 && totalPriceMonthlySubscriptionNegative === 0
                            ) ? (
                              <p>Cancel</p>
                            ) : (
                          ...
                          <td>
                            ...
                            <p>
                              <div className="double-value-cell">
                                ...
                                data.reduce((acc, item) => acc + item.merchants_monthly, 0) === 0 ? (
                                  <span></span>
                                ) : (
                                  <>
                                    <span>
                                      {data
                                        .reduce((acc, item) => acc + item.merchants_monthly, 0)
                                        .toLocaleString('en-US')}
                                    </span>
                                    <div className="divider" />
                                    <span>{totalMerchantsMonthlySubscription.toLocaleString('en-US')}</span>
                                  </>
                                )}
                              </div>
                            </p>
                            <p>
                              <div className="double-value-cell">
                                <span>
                                  {formatPercent(
                                    data.reduce((acc, item) => acc + item.merchants_monthly, 0) /
                                ...
                                <span>
                                  {formatPercent(
                                    totalMerchantsMonthlySubscription / getTotalMerchants(valueSubscription),
                                  )}
                                </span>
                              </div>
                            </p>
                          ...
                          <td>
                            <p>
                              <div className="double-value-cell">
                                ...
                                totalPriceMonthlySubscriptionNegative === 0 ? (
                                  <span style={{ minWidth: 60, textAlign: 'right', marginTop: '15px' }}> </span>
                                ) : (
                                  <>
                                    <span>${totalPriceMonthlyEarningNegative}</span>
                                    <div className="divider" />
                                    <span>${totalPriceMonthlySubscriptionNegative}</span>
                                  </>
                                )}
                              </div>
                            </p>
                            <p>
                              <div className="double-value-cell">
                                ...
                                data.reduce((acc, item) => acc + item.rev_monthly, 0) === 0 ? (
                                  <span></span>
                                ) : (
                                  <>
                                    <span>
                                      ${data.reduce((acc, item) => acc + item.rev_monthly, 0).toLocaleString('en-US')}
                                    </span>
                                    <div className="divider" />
                                    <span>${totalRevMonthlySubscription.toLocaleString('en-US')}</span>
                                  </>
                                )}
                              </div>
                            </p>
                            <p>
                              <div className="double-value-cell">
                                <span>
                                  {formatPercent(
                                    data.reduce((acc, item) => acc + item.rev_monthly, 0) / getTotalRev(valueEarning),
                                  )}
                                </span>
                                <div className="divider" />
                                <span>
                                  {formatPercent(totalRevMonthlySubscription / getTotalRev(valueSubscription))}
                                </span>
                              </div>
                            </p>
                          </td>
              ...
              <thead>
                ...
                <tr>
                  {headersYearly.map(({ key, label, tooltip }) => (
                    <th key={key} onClick={() => handleSortYearly(key)} style={{ cursor: 'pointer' }}>
                      {label} {getTooltipContent(tooltip)}{' '}
                      <SortIcon active={sortColumnYearly === key} direction={sortDirectionYearly} />
                    </th>
                  ))}
                </tr>
              </thead>
                    ...
                    if (hasData) {
                      ...
                      rows.push(
                        <tr key={index}>
                          ...
                          <td>
                            <div className="double-value-cell">
                              {item.merchants_yearly_subscription === 0 && item.merchants_yearly === 0 ? (
                                <span></span>
                              ) : (
                                <>
                          ...
                          <td>
                            <div className="double-value-cell">
                              {item.rev_yearly_subscription === 0 && item.rev_yearly === 0 ? (
                                <span></span>
                              ) : (
                                <>
                                  <span>${item.rev_yearly.toLocaleString('en-US')}</span>
                                  <div className="divider" />
                                  <span>${item.rev_yearly_subscription.toLocaleString('en-US')}</span>
                                </>
                              )}
                            </div>
                          </td>
                          <td>
                            <div className="double-value-cell">
                              {isZeroPercent(item.percent_yearly_subscription) && isZeroPercent(item.percent_yearly) ? (
                                <span></span>
                              ) : (
                                <>
                  ...
                  return (
                    <>
                      ...
                      {hasAnyData ? (
                        <tr className="sum-row">
                          <td>
                            {!(totalPriceYearlyEarningNegative === 0 && totalPriceYearlySubscriptionNegative === 0) ? (
                              <p>Cancel</p>
                            ) : (
                              <p className="no-data"></p>
                            )}
                          ...
                          <td>
                            ...
                            <p>
                              <div className="double-value-cell">
                                ...
                                data.reduce((acc, item) => acc + item.merchants_yearly, 0) === 0 ? (
                                  <span></span>
                                ) : (
                                  <>
                                    <span>
                                      {data
                                        .reduce((acc, item) => acc + item.merchants_yearly, 0)
                                        .toLocaleString('en-US')}
                                    </span>
                                    <div className="divider" />
                                    <span>{totalMerchantsYearlySubscription.toLocaleString('en-US')}</span>
                                  </>
                                )}
                              </div>
                            </p>
                            <p>
                              <div className="double-value-cell">
                                <span>
                                  {(
                                    (data.reduce((acc, item) => acc + item.merchants_yearly, 0) /
                                      getTotalMerchants(valueEarning)) *
                                    100
                                  )
                                    .toFixed(2)
                                ...
                                <div className="divider" />
                                <span>
                                  {((totalMerchantsYearlySubscription / getTotalMerchants(valueSubscription)) * 100)
                                    .toFixed(2)
                                    .toLocaleString('en-US')}
                                  %
                                </span>
                          ...
                          <td>
                              ...
                              <div className="double-value-cell">
                                {totalPriceYearlyEarningNegative === 0 && totalPriceYearlySubscriptionNegative === 0 ? (
                                  ...
                                  <>
                                    ...
                                    <div className="divider" />
                                    <span>${totalPriceYearlySubscriptionNegative}</span>
                                  </>
                                )}
                              </div>
                            </p>
                            <p>
                              <div className="double-value-cell">
                                ...
                                data.reduce((acc, item) => acc + item.rev_yearly, 0) === 0 ? (
                                  <span></span>
                                ) : (
                                  <>
                                    <span>
                                      ${data.reduce((acc, item) => acc + item.rev_yearly, 0).toLocaleString('en-US')}
                                    </span>
                                    <div className="divider" />
                                    <span>${totalRevYearlySubscription.toLocaleString('en-US')}</span>
                                  </>
                                )}
                              </div>
                            </p>
                            <p>
                              <div className="double-value-cell">
                                <span>
                                  {(
                                    (data.reduce((acc, item) => acc + item.rev_yearly, 0) / getTotalRev(valueEarning)) *
                                    100
                                  )
                                    .toFixed(2)
                                    .toLocaleString('en-US')}
                                ...
                                <div className="divider" />
                                <span>
                                  {((totalRevYearlySubscription / getTotalRev(valueSubscription)) * 100)
                                    .toFixed(2)
                                    .toLocaleString('en-US')}
                                  %
                                </span>
