...
export default function ListAppByDay({ initialData }) {
    ...
    const handleChangeSortBy = (value) => {
        ...
        router.push(
            view_type == 'bfs'
                ? `${pathname}?sort_by=${value}&date=${date}&type=${view_type}&page=${page}&per_page=${perPage}`
                : `${pathname}?sort_by=${value}&date=${date}&page=${page}&per_page=${perPage}`,
        );
    ...
    const onChangePage = (page, per_page) => {
        router.push(
            view_type == 'bfs'
                ? `${pathname}?sort_by=${sort_by}&date=${date}&type=${view_type}&page=${page}&per_page=${per_page}`
                : `${pathname}?sort_by=${sort_by}&date=${date}&page=${page}&per_page=${per_page}`,
        );
        asyncFetch(sort_by, date, page, per_page);
    ...
    const columns = [
                        ...
                        <div className="item-detail-app">
                            <div className="name-app-shopify">
                                {isAppInfo(record, 'before_rank') &&
                                    ...
                                    isAppInfo(record, 'before_rank') - isAppInfo(record, 'rank') > 0 && (
                                        <span className="calular-incre">
                                            <UpOutlined />{' '}
                                            {isAppInfo(record, 'before_rank') - isAppInfo(record, 'rank')}{' '}
                                        </span>
                                    )}
                                {isAppInfo(record, 'before_rank') &&
                                    isAppInfo(record, 'rank') &&
                                    isAppInfo(record, 'before_rank') - isAppInfo(record, 'rank') < 0 && (
                                        <span className="calular-decre">
                                            <DownOutlined />{' '}
                                            {isAppInfo(record, 'rank') - isAppInfo(record, 'before_rank')}{' '}
                                        </span>
                                    )}
                                ...
                                <div className="tagline">
                                    {isAppInfo(record, 'tagline')
                                        ? isAppInfo(record, 'tagline')
                                        : isAppInfo(record, 'metatitle')}
                                </div>
        ...
        },
        {
            ...
            render: (review_count, record) =>
                isAppInfo(record, 'review_count') > 0
                    ? isAppInfo(record, 'review_count')
                    : isAppInfo(record, 'review') || null,
            width: 100,
        },
    ];
