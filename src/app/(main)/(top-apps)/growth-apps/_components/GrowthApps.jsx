...
export default function GrowthApps({ initialDataTopGrowth }) {
    ...
    const renderGrowthInfo = (current, before) => (
        ...
        >
            ...
            {Math.abs(current - before) > 0 && !isNaN(Math.abs(current - before))
                ? formatStarValue(current, before)
                : null}
        </span>
    );
    ...
    const columns = [
                    ...
                    {categories.map((cat, index) => (
                        <Row key={index} style={{ lineHeight: '30px' }}>
                            ...
                            <Col span={11}>
                                ...
                                {cat.current_rank &&
                                    cat.before_rank &&
                                    renderGrowthInfo(cat.current_rank, cat.before_rank)}
                            </Col>
                        </Row>
                    ))}
                    {record.bfs_info && (
        ...
        },
        {
            ...
            render: (total, record) => (
                <>
                    {record.categories
                        ? renderTagType(record.categories.reduce((sum, item) => sum + item.rank_change, 0))
                        : ''}
                </>
            ),
            width: 50,
                ...
                <div className="detail-top-growth-app-body container">
                    ...
                    <div className="container-filter-range">
                        <div className="dashboard-range">
                            <RangePicker
                                defaultValue={[
                                    dayjs(start_date.current, dateFormat),
                                    dayjs(end_date.current, dateFormat),
                                ]}
                                format={dateFormat}
                                allowClear={false}
                                ...
                                <TreeSelect
                                    ...
                                    filterTreeNode={(inputValue, treeNode) => {
                                        ...
                                        const titleMatches = searchTerms.every((term) =>
                                            treeNode.title.toLowerCase().includes(term),
                                        );
                                        return titleMatches;
                                    }}
                                />
