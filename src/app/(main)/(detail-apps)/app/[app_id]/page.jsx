...
export default async function DetailApp({ params }) {
    ...
    try {
        const [appDetail, ratingChange, reviewsChange, changeLog, dataCategoryPos, dataCategory, dataCollection] =
            await Promise.all([
                token
                    ? DetailAppApiService.getInfoAppLoggedServer(app_id, token)
                    : DetailAppApiService.getAppInfo(app_id),
        ...
        if (appDetail && appDetail.data && appDetail.data.detail) {
            ...
            appDetail.competitor?.map((item) => {
                const activeKey =
                    (initialPanes && initialPanes.length ? +initialPanes[initialPanes.length - 1].key : 0) + 1;
                initialPanes.push({
                    appId: item.app_id,
                    title: {
                        ...item,
