...
export default function ProductPage({
    ...
    useEffect(() => {
        const watchingAppData = {
            app_id: dataInfoApps?.data?.app_id,
        };
        ...
        return (
            <div className="faq-section">
                ...
                <Collapse
                    ...
                    expandIcon={({ isActive }) =>
                        isActive ? (
                            <MinusOutlined className="faq-icon faq-active" />
                        ) : (
                ...
                >
                    <Panel header={`1. Is ${dataInfoApps.data.detail.name} free on Shopify?`} key="1">
                        {hasFreePlan ? (
                            ...
                            <p>
                                No. {dataInfoApps.data.detail.name} pricing starts at {minPricing}. For more details
                                about {dataInfoApps.data.detail.name} pricing, check
                                <MyLink href={`/app/${dataInfoApps.data.detail.app_id}/pricing`}> here</MyLink>.
                            </p>
                        )}
                    </Panel>
                    <Panel header={`2. How do I install ${dataInfoApps.data.detail.name} on my Shopify store?`} key="2">
                        <p>
                            - Navigate to the Shopify App Store, search for {dataInfoApps.data.detail.name}, and click
                            “Add app.” <br />
                    ...
                    {dataInfoApps.data.detail.languages && dataInfoApps.data.detail.languages.length > 0 ? (
                        <Panel header={`3. What languages does ${dataInfoApps.data.detail.name} support?`} key="3">
                            <p>
                                {dataInfoApps.data.detail.name} supports:{' '}
                                {dataInfoApps.data.detail?.languages?.join(', ')}
                            </p>
                        </Panel>
                    ) : null}
                    ...
                    {dataInfoApps.data.detail.integrations && dataInfoApps.data.detail.integrations.length > 0 ? (
                        <Panel
                            header={`4. Which App/Tool does ${dataInfoApps.data.detail.name} integrate with?`}
                            key="4"
                        >
                            <p>
