...
import { getValidPreviousUrl } from '@/utils/helpers/url';
...
export default function LoginPage() {
    ...
    const storedPreviousUrl = Auth.getPreviousUrl();
    const previousUrl = getValidPreviousUrl(storedPreviousUrl);

    const handleSubmit = async (values) => {
        const resultAction = await dispatch(loginApps(values));
        if (loginApps.fulfilled.match(resultAction) && resultAction.payload.code === 0) {
            router.refresh();
        }
    };
    ...
    }, [user, accessToken, refreshToken, username, accessStore, router, previousUrl, hasPassword]);
            ...
            <AuthForm>
                ...
                <div className="LoginApp-description">
                    ...
                    <span>
                        ...
                        <MyLink href={`${LayoutPaths.Auth}${Paths.Register}`}>Sign Up</MyLink>
                    </span>
                </div>
